    ...
    <section className="rounded-2xl border border-stone-200 bg-white/80 p-5 shadow-sm">
      ...
      <form onSubmit={save} className="mt-5 space-y-4">
        ...
        <div className="rounded-xl bg-stone-50 p-4">
          ...
          <p className="mt-1 text-sm text-stone-800">{account.email}</p>
          <div className="mt-2 flex flex-wrap items-center gap-3">
            <span className={`rounded-full px-2 py-1 text-xs font-medium ${account.emailVerified ? "bg-emerald-100 text-emerald-700" : "bg-amber-100 text-amber-700"}`}>
              {account.emailVerified ? "Verified" : "Verification pending"}
            </span>
            {!account.emailVerified ? <button type="button" disabled={saving} onClick={() => void resend()} className="text-sm font-medium text-[#b75536] disabled:opacity-50">Resend verification</button> : null}
          </div>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <Link href="/settings/models" className="text-sm font-medium text-stone-600 hover:text-stone-900">Manage models and pricing</Link>
        </div>
      </form>
    </section>
  );
