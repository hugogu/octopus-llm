...
Add an administrator-controlled anonymous-access flag to platform-owned configured models, expose a policy-filtered public model catalogue, and provide an ephemeral anonymous chat stream that never creates a server-owned conversation. The browser will keep anonymous conversations locally and send the selected conversation context with each anonymous turn. After registration, the frontend will import local conversations into authenticated sessions through an idempotent, per-conversation transaction.
